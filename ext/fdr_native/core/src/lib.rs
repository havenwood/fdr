...
struct EntryFilters {
    ...
    /// Base directory joined onto relative paths so full-path matching sees
    /// absolute paths, as in fd. `Some` only when `SearchConfig::full_path`.
...
impl EntryFilters {
    fn new(config: &SearchConfig) -> Result<Self, SearchError> {
        let full_path_base = if config.full_path {
...
fn configure_walker(
    ...
    builder
        ...
        // Cap workers at 64 like fd, to bound startup overhead on machines
        // with many cores; the ignore crate's own default caps at 12.
        .threads(
...
/// Entries yielded serially before falling back to the parallel walker, whose
/// idle workers poll on millisecond sleeps that dwarf a small tree's walk.
const PARALLEL_THRESHOLD: usize = 1024;

/// Grep scans file contents per entry, so it bails to the parallel walker
/// far sooner than search does.
const GREP_PARALLEL_THRESHOLD: usize = 64;

/// Serial grep also bails once the files it would scan exceed this many
/// bytes, since discarded pre-scan work grows with file size, not entry
/// count.
...
/// Matching lines in `path`, or `None` when it is binary or has no match.
fn grep_file(
...
fn serial_grep(
    ...
    for (visited, entry) in builder.build().enumerate() {
        ...
        if let Some(result) = grep_file(&mut searcher, matcher, entry.path()) {
            results.push(result);
        }
    }
...
pub fn grep_with_cancel(
    ...
    walker.run(|| {
        ...
        Box::new(move |entry| {
            ...
            if let Some(result) = grep_file(&mut searcher, matcher.as_ref(), entry.path()) {
                drop(tx.send(result));
            }

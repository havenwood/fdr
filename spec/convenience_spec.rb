...
describe "Seen.each_path" do
  describe "basic searches" do
    ...
    it "supports type filter" do
      results = path_results(pattern: "lib", paths: ["."], type: "d", max_depth: 2)
    ...
    it "finds with single path" do
      results = path_results(pattern: "seen", paths: ["lib"])
  ...
  describe "edge cases" do
    ...
    it "handles nonexistent paths gracefully" do
      results = path_results(paths: ["/nonexistent/path/12345"])

      assert_empty results, "should yield nothing for a nonexistent path"
    end

    it "accepts empty exclude array" do
      results_no_exclude = path_results(extension: "rb", paths: ["lib"])
      results_empty_exclude = path_results(extension: "rb", paths: ["lib"], exclude: [])

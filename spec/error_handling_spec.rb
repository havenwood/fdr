...
describe "Seen error handling" do
  describe "invalid paths" do
    it "handles nonexistent paths gracefully" do
      results = path_results(paths: ["/nonexistent/path/xyz123"])
  ...
  describe "unreadable entries" do
    ...
    it "skips what it cannot read by default, as fd does" do
      with_unreadable do |dir|
        assert_includes path_results(paths: [dir]), File.join(dir, "ok.txt")
        assert_equal [File.join(dir, "ok.txt")], line_results(pattern: "needle", paths: [dir]).keys
      end
    end
    ...
    it "raises for an unreadable entry when ignore_error is false" do
      with_unreadable do |dir|
        error = assert_raises(Seen::IOError) do
          Seen.each_path(paths: [dir], ignore_error: false).to_a
        end

        assert_match(/secret/, error.message)
        assert_raises(Seen::IOError) do
          Seen.each_line(pattern: "needle", paths: [dir], ignore_error: false).to_a
        end
      end
    end
    ...
    it "treats nil ignore_error as falsey" do
      with_unreadable do |dir|
        assert_raises(Seen::IOError) do
          Seen.each_path(paths: [dir], ignore_error: nil).to_a
        end
        assert_raises(Seen::IOError) do
          Seen.each_line(pattern: "needle", paths: [dir], ignore_error: nil).to_a
        end
      end
    end

    it "does not raise on a readable tree when ignore_error is false" do
      refute_empty Seen.each_path(paths: ["lib"], ignore_error: false).to_a
    end

    it "raises for an explicit non-file when ignore_error is false" do
      Dir.mktmpdir("seen-non-file") do |dir|
        fifo = File.join(dir, "fifo")
        File.mkfifo(fifo)

        assert_empty Seen.each_line(pattern: "needle", paths: [fifo]).to_a
        assert_empty Seen.each_line(pattern: "needle", paths: [dir], ignore_error: false).to_a
        [{}, {column: true}, {byte_range: true}].each do |position|
          error = assert_raises(Seen::IOError) do
            Seen.each_line(pattern: "needle", paths: [fifo], ignore_error: false, **position).to_a
          end
    ...
    it "raises when grep cannot open a file and ignore_error is false" do
      Dir.mktmpdir("seen-unreadable-file") do |dir|
        ...
        [{}, {column: true}, {byte_range: true}].each do |position|
          error = assert_raises(Seen::IOError) do
            Seen.each_line(pattern: "needle", paths: [path], ignore_error: false, **position).to_a
          end
  ...
  describe "invalid patterns" do
    it "handles empty pattern by matching all files" do
      empty_pattern = path_results(pattern: "", paths: ["lib"], max_depth: 1)
      all_files = path_results(paths: ["lib"], max_depth: 1)
    ...
    it "handles nil pattern by matching all files" do
      nil_pattern = path_results(pattern: nil, paths: ["lib"], max_depth: 1)
      all_files = path_results(paths: ["lib"], max_depth: 1)
    ...
    it "raises error for invalid regex pattern" do
      error = assert_raises(RegexpError) do
        path_results(pattern: "[invalid(regex", paths: ["."], max_depth: 1)
      end
      assert_match(/Path search failed/, error.message)
    end

    it "raises error for unclosed bracket in regex" do
      error = assert_raises(RegexpError) do
        path_results(pattern: "[abc", paths: ["."], max_depth: 1)
      end
      assert_match(/Path search failed/, error.message)
    end
    ...
    it "raises error for invalid named group in regex" do
      error = assert_raises(RegexpError) do
        path_results(pattern: "(?P<invalid)", paths: ["."], max_depth: 1)
      end
      assert_match(/Path search failed/, error.message)
    end
    ...
    it "raises error for invalid glob pattern" do
      error = assert_raises(ArgumentError) do
        path_results(pattern: "[invalid", paths: ["."], glob: true, max_depth: 1)
      end
      assert_match(/Path search failed/, error.message)
    end
    ...
    it "raises error for invalid exclude pattern" do
      error = assert_raises(ArgumentError) do
        path_results(paths: ["."], exclude: ["[invalid"], max_depth: 1)
      end
      assert_match(/Path search failed/, error.message)
    end
  end
  ...
  describe "invalid argument types" do
    it "raises TypeError when paths is not an Array" do
      assert_raises(TypeError) do
        path_results(paths: "lib")
      end
    end

    it "raises TypeError when extension is not a String" do
      assert_raises(TypeError) do
        path_results(paths: ["lib"], extension: :rb)
      end
    end

    it "raises TypeError when max_depth is not an Integer" do
      assert_raises(TypeError) do
        path_results(paths: ["lib"], max_depth: "1")
      end
    end

    it "raises TypeError when max_depth is a Float" do
      assert_raises(TypeError) do
        path_results(paths: ["lib"], max_depth: 2.9)
      end
    end

    it "raises TypeError when min_size is a Float" do
      assert_raises(TypeError) do
        path_results(paths: ["lib"], min_size: 1.5)
      end
    end

    it "raises TypeError when changed_within is a Float" do
      assert_raises(TypeError) do
        path_results(paths: ["lib"], changed_within: 0.5)
      end
    end

    it "raises TypeError when exclude is not an Array" do
      assert_raises(TypeError) do
        path_results(paths: ["lib"], exclude: "vendor")
      end
    end

    it "raises RangeError when min_size exceeds the native integer range" do
      assert_raises(RangeError) do
        path_results(paths: ["lib"], min_size: 2**70)
      end
    end

    it "raises TypeError when grep pattern is not a String" do
      assert_raises(TypeError) do
        line_results(pattern: 42, paths: ["lib"])
      end
    end

    it "raises Seen::InvalidType when grep pattern is nil" do
      assert_raises(Seen::InvalidType) do
        line_results(pattern: nil, paths: ["lib"])
      end
    end

    it "accepts truthy values for boolean kwargs" do
      Dir.mktmpdir("seen-truthy") do |dir|
        File.write(File.join(dir, ".hidden.txt"), "")
        with_string = path_results(paths: [dir], hidden: "yes")
        with_true = path_results(paths: [dir], hidden: true)
  ...
  describe "invalid size and time values" do
    it "raises error for negative min_size" do
      error = assert_raises(ArgumentError) { path_results(paths: ["."], min_size: -1) }
      assert_match(/min_size must be a non-negative integer/, error.message)
    end

    it "raises error for negative max_size" do
      error = assert_raises(ArgumentError) { path_results(paths: ["."], max_size: -1) }
    ...
    it "raises error for negative changed_within" do
      error = assert_raises(ArgumentError) { path_results(paths: ["."], changed_within: -1) }
      assert_match(/changed_within must be a non-negative integer/, error.message)
    end

    it "raises error for negative changed_before" do
      error = assert_raises(ArgumentError) { path_results(paths: ["."], changed_before: -1) }
  ...
  describe "error classes" do
    it "tags the errors it raises itself while keeping their stdlib class" do
      [
        [Seen::InvalidPattern, RegexpError, -> { path_results(pattern: "[", paths: ["lib"]) }],
        [Seen::InvalidOption, ArgumentError, -> { path_results(type: "nope", paths: ["lib"]) }],
        [Seen::InvalidOption, ArgumentError, -> { path_results(type: %w[f nope], paths: ["lib"]) }],
        [Seen::InvalidType, TypeError, -> { path_results(paths: nil) }],
        [Seen::InvalidType, TypeError, -> { path_results(exclude: [42], paths: ["lib"]) }],
        [Seen::InvalidType, TypeError, -> { path_results(type: 42, paths: ["lib"]) }],
        [Seen::InvalidType, TypeError, -> { path_results(type: [:f, 42], paths: ["lib"]) }],
        [Seen::InvalidType, TypeError, -> { path_results(extension: [42], paths: ["lib"]) }],
        [Seen::OutOfRange, RangeError, -> { path_results(max_depth: 2**64, paths: ["lib"]) }]
      ].each do |seen_class, stdlib_class, call|
        error = assert_raises(seen_class) { call.call }
    ...
    it "raises Seen::IOError when the working directory is gone" do
      skip "fork is unavailable" unless Process.respond_to?(:fork)

      reader, writer = IO.pipe
      pid = fork do
        reader.close
        dir = Dir.mktmpdir("seen-gone")
        Dir.chdir(dir)
        FileUtils.remove_entry(dir)
        begin
          path_results(pattern: "x", full_path: true, paths: ["."])
          writer.puts "no raise"
        rescue => e
          writer.puts "#{e.class} #{e.is_a?(Seen::Error)} #{e.is_a?(IOError)}"
        end
        writer.close
        exit! 0
      end
      ...
      assert_equal "Seen::IOError true true", result
    end

    it "does not tag Ruby's own keyword errors" do
      refute_kind_of Seen::Error, assert_raises(ArgumentError) { path_results(nope: true) }
      refute_kind_of Seen::Error, assert_raises(ArgumentError) { line_results(paths: []) }
    end
    ...
    it "leaves an exception raised by the caller's own coercion intact" do
      ...
      error = assert_raises(boom) { path_results(pattern: raiser.new, paths: ["lib"]) }
    ...
    it "accepts private implicit conversion methods" do
      ...
      assert_equal path_results(pattern: "seen", paths: ["lib"]),
        path_results(pattern: stringish.new, paths: ["lib"])
      assert_equal path_results(paths: ["lib"]), path_results(paths: arrayish.new)
      assert_equal path_results(paths: ["lib"]), path_results(paths: [pathish.new])
      assert_equal path_results(paths: ["lib"], extension: "rb"),
        path_results(paths: ["lib"], extension: extensions.new)
    end
  ...
  describe "invalid options" do
    it "raises for a blank exclude glob, even with no paths" do
      assert_raises(ArgumentError) { path_results(paths: ["lib"], exclude: [""]) }
      assert_raises(ArgumentError) { path_results(paths: ["lib"], exclude: ["  "]) }
      assert_raises(ArgumentError) { path_results(paths: [], exclude: [""]) }
    end

    it "raises error for unknown file types" do
      error = assert_raises(ArgumentError) do
        path_results(type: "invalid", paths: ["."], max_depth: 1)
      end
    ...
    it "strips leading dots from extension" do
      with_dot = path_results(extension: ".rb", paths: ["lib"], max_depth: 1)
      without_dot = path_results(extension: "rb", paths: ["lib"], max_depth: 1)
      refute_empty with_dot, "a dotted extension should match like the bare extension"
      assert_equal without_dot, with_dot
      assert_equal without_dot, path_results(extension: "..rb", paths: ["lib"], max_depth: 1)
    end

    it "treats an empty extension as a trailing dot, as fd does" do
      Dir.mktmpdir("seen-empty-extension") do |dir|
        trailing_dot = File.join(dir, "trailing.")
        File.write(trailing_dot, "")
        File.write(File.join(dir, "ordinary"), "")

        assert_equal [trailing_dot], path_results(extension: "", paths: [dir])
        assert_equal [trailing_dot], path_results(extension: ".", paths: [dir])
      end
    end

    it "handles nil extension by ignoring the filter" do
      with_nil = path_results(extension: nil, paths: ["lib"], max_depth: 1)
      without_ext = path_results(paths: ["lib"], max_depth: 1)
  ...
  describe "edge cases" do
    ...
    it "handles very deep max_depth values by finding files" do
      results = path_results(paths: ["lib"], max_depth: 1000)
    ...
    it "handles special characters in patterns" do
      results = path_results(pattern: 'spec_helper\.rb$', paths: ["spec"], max_depth: 1)
    ...
    it "handles Unicode patterns" do
      Dir.mktmpdir do |dir|
        ...
        results = path_results(pattern: "サンプル", paths: [dir])
  ...
  describe "permission errors" do
    it "continues searching when encountering permission errors" do
      ...
      Dir.mktmpdir do |dir|
        ...
        begin
          results = path_results(paths: [dir])
        ensure

...
describe "Symlink behavior" do
  before do
    @tmpdir = Dir.mktmpdir("seen_symlink_test")
  ...
  describe "follow: false (default)" do
    it "does not traverse into symlinked directories" do
      results = path_results(paths: [@tmpdir], follow: false)
    ...
    it "finds symlinks themselves when type is not specified" do
      results = path_results(paths: [@tmpdir], follow: false, max_depth: 1)

      assert_includes results, @link_to_dir
    end

    it "does not follow symlinked files" do
      results = path_results(paths: [@tmpdir], pattern: "link_to_file", follow: false)
  ...
  describe "follow: true" do
    it "traverses into symlinked directories" do
      results = path_results(paths: [@tmpdir], follow: true)
    ...
    it "follows symlinks to files" do
      results = path_results(paths: [@tmpdir], pattern: "real_file", follow: true)
  ...
  describe 'type: "l" (symlink)' do
    it "finds only symlinks" do
      results = path_results(paths: [@tmpdir], type: "l", max_depth: 1)
    ...
    it "finds directory symlinks" do
      results = path_results(paths: [@tmpdir], type: "l", pattern: "link_to_dir", max_depth: 1)
    ...
    it "finds file symlinks" do
      results = path_results(paths: [@tmpdir], type: "l", pattern: "link_to_file", max_depth: 1)
    ...
    it "works with symlink type alias" do
      results = path_results(paths: [@tmpdir], type: "symlink", max_depth: 1)
  ...
  describe "symlink as the search root" do
    it "omits the root link and returns its children" do
      results = path_results(paths: [@link_to_dir])
    ...
    it "omits the root link and returns its children with follow: true" do
      results = path_results(paths: [@link_to_dir], follow: true)
    ...
    it 'finds a file symlink root with type: "l"' do
      results = path_results(paths: [@link_to_file], type: "l")
    ...
    it "sizes a file symlink root by the link, not its target" do
      ...
      assert_empty path_results(paths: [link], min_size: 20_000)
      assert_equal [link], path_results(paths: [link], min_size: 20_000, follow: true)
    end
    ...
    it "answers the same for a symlink root on both sides of the parallel threshold" do
      ...
      serial = path_results(paths: [pad, link], min_size: 20_000, follow: true)
      70.times { |i| Dir.mkdir(File.join(pad, "d#{i}")) }
      parallel = path_results(paths: [pad, link], min_size: 20_000, follow: true)
  ...
  describe "symlink cycles with follow: true" do
    it "terminates on a self-referential directory link in either walker" do
      ...
      serial = Timeout.timeout(15) { path_results(paths: [cycle], follow: true) }
      70.times { |i| Dir.mkdir(File.join(cycle, "d#{i}")) }
      parallel = Timeout.timeout(15) { path_results(paths: [cycle], follow: true) }
    ...
    it "terminates on a two-directory cycle in either walker" do
      ...
      serial = Timeout.timeout(15) { path_results(paths: [first], follow: true) }
      70.times { |i| Dir.mkdir(File.join(second, "d#{i}")) }
      parallel = Timeout.timeout(15) { path_results(paths: [first], follow: true) }
  ...
  describe "combination of follow and type" do
    it 'type: "l" with follow: true matches only broken symlinks' do
      results = path_results(paths: [@tmpdir], type: "l", follow: true, max_depth: 1)
    ...
    it 'type: "l" with follow: false finds symlinks' do
      results = path_results(paths: [@tmpdir], type: "l", follow: false, max_depth: 1)
    ...
    it 'type: "f" excludes symlinks with follow: false' do
      results = path_results(paths: [@tmpdir], type: "f", follow: false)
  ...
  describe "pattern matching on symlinks" do
    it "matches pattern on symlink names" do
      results = path_results(paths: [@tmpdir], pattern: "link_to", max_depth: 1)
    ...
    it "can filter symlinks by extension" do
      ...
      results = path_results(paths: [@tmpdir], type: "l", extension: "rb", max_depth: 1)
